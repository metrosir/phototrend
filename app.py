                    ...
                    with gr.Tabs():
                        with gr.TabItem("参数(Params)"):
                            ...
                            with gr.Row():
                                ...
                                with gr.Column():
                                    batch_count = gr.Slider(minimum=1, step=1, label='生成数量(Batch count)', value=1, lines=1, elem_id="txt2img_batch_count")
            ...
            with gr.Column():
                with gr.Row():
                    with gr.Tabs():
                        with gr.TabItem("投影"):
                            with gr.Row():
                                x_offset=gr.Slider(minimum=-600, maximum=600, step=1, label='X轴偏移', value=constant.shadow['plane']['left']['x'], elem_id="x_offset")
                                y_offset=gr.Slider(minimum=-600, maximum=600, step=1, label='Y轴偏移', value=constant.shadow['plane']['left']['y'], elem_id="y_offset")
                                blur=gr.Slider(minimum=0, maximum=100, step=1, label='模糊半径', value=constant.shadow['plane']['left']['blur'], elem_id="blur")
                                opacity=gr.Slider(minimum=0, maximum=100, step=0.01, label='不透明度', value=constant.shadow['plane']['left']['opacity'], elem_id="opacity")
                        ...
                        with gr.TabItem("透视投影"):
                            with gr.Row():
                                ...
                                p_angle = gr.Slider(minimum=-360, maximum=360, step=1, label='角度', value=constant.shadow['perspective']['left']['angle'], elem_id="p_angle")
                                p_distance =gr.Slider(minimum=0, maximum=24, step=0.1, label='水平距离', value=constant.shadow['perspective']['left']['distance'], elem_id="p_distance")
                                p_length = gr.Slider(minimum=0, maximum=24, step=0.01, label='相对长度', value=constant.shadow['perspective']['left']['length'], elem_id="p_length")
                                p_blur = gr.Slider(minimum=0, maximum=100, step=1, label='模糊半径', value=constant.shadow['perspective']['left']['blur'], elem_id="p_blur")
                                p_opacity=gr.Slider(minimum=0, maximum=100, step=1, label='不透明度', value=constant.shadow['perspective']['left']['opacity'], elem_id="p_opacity")
                                p_gradient_strength =gr.Slider(minimum=0, maximum=10, step=1, label='渐变强度', value=constant.shadow['perspective']['left']['gradient'], elem_id="p_gradient_strength")
                                ...
                                p_interpolation = gr.Dropdown(label='插值', choices=['最近邻插值', '线性插值', '立方插值', '无晕圈插值', '低晕圈插值'], elem_id="p_interpolation", value="最近邻插值", interactive=True, type="index")
                                p_allow_resize = gr.Checkbox(label=f'允许改变大小', value=constant.shadow['perspective']['left']['allow_resize']
